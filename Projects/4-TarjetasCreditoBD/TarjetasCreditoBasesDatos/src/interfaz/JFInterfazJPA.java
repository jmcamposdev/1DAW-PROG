...
import JPA.TarjetaCreditoDAO;
import JPA.TarjetaCreditoJPA;
 ...
 * @author josemaria
 */
public class JFInterfazJPA extends javax.swing.JFrame {

    /**
     * Creates new form JFInterfaz
     */
    public JFInterfazJPA() {
        initComponents();
        inicializar();
    }

    /**
     * Método privado el cual inicializa toda la interfaz,
     * crea los modelos de los JTable (Tarjeta y Movimientos)
     * Carga todas las tarjetas de la BBDD a través de JPA
     */
    private void inicializar() {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("TarjetasCreditoBasesDatosPU");
        // Creamos la conexión con la BBDD
        this.conexionTarjetaCreditoDAO = new TarjetaCreditoDAO(emf);
        ...
        this.listaTarjetas = new ArrayList<>(conexionTarjetaCreditoDAO.findTarjetaCreditoJPAEntities()); // Cargamos todas las
        // Tarjetas de la BBDD
    ...
    /**
     * Método que se ejecuta al realizar click a una tarjeta del JTable
     * Muestra un menu si realiza click a una Tarjeta (Eliminar, Realizar Pago, Modificar PIN)
     * Si realiza click derecho sobre varias tarjetas seleccionada muestra (Eliminar Tarjetas)
     * Si realiza doble click a una Tarjeta muestra sus datos en el JTabbedPane
    ...
    /**
     * Acción llamada cuando se realiza click en JButton "Añadir Tarjeta"
     * Llama al método añadirTarjetaRecursivo().
     * @param evt Action Event
     */
    ...
    /**
     * Acción llamada cuando se realiza click en JPopPupMenuItem "Eliminar Tarjeta"
     * Llama al método eliminarTarjetas().
     * @param evt Action Event
     */
    ...
    /**
     * Acción llamada cuando se realiza click en JPopPupMenuItem "Eliminar Tarjetas"
     * Llama al método eliminarTarjetas().
     * @param evt Action Event
     */
    ...
    /**
     * Acción llamada cuando se realiza click en JMenuItem "Modificar PIN"
    ...
    /**
     * Acción llamada cuando se realiza click en JButton "Modificar PIN"
     * Llama al método  modificarPIN();
     * @param evt Action Event
     */
    ...
    /**
     * Acción llamada cuando se realiza click en JMenuItem "Realizar Pago"
    ...
    /**
     * Acción llamada cuando se realiza click en JButton "Realizar Pago"
     * Llama al método realizarPago();
     * @param evt Action Event
     */
    ...
    /**
     * Método recursivo el cual muestra un JDialog para crear una Tarjeta comprueba
     * que no esté duplicada en caso afirmativo vuelve a mostrar el JDialog
     * con los campos rellenos de los datos del usuario para que los cambie y 
     * si no está duplicada se procede a cargarla en la BBDD y en el programa
    ...
    private void añadirTarjetaRecursivo(TarjetaCredito tarjetaExistente) {
        ...
        if (jDCrearTarjeta.isCreada()) { // Si ha pulsado en "Crear" en el JDialog
            TarjetaCreditoJPA nuevaTarjeta = new TarjetaCreditoJPA(jDCrearTarjeta.getTarjetaCredito()); // Obtenemos la Nueva Tarjeta
            ...
            if (listaTarjetas.contains(nuevaTarjeta)) { // Si ya existe otra tarjeta con el mismo número
                ...
                try {
                    conexionTarjetaCreditoDAO.create(nuevaTarjeta);
    ...
    /**
     * Método privado el cual elimina una o varias tarjetas seleccionadas.
     * Si el usuario ha seleccionado una tarjeta o tiene seleccionado una
     * se eliminará y si tiene multiples tarjetas seleccionadas se eliminarán 
     * todas las seleccionadas. Antes de realizar la eliminación se pide al 
     * usuario confirmación de la Acción. Si la confirma se procede a eliminar las tarjetas
     */
    private void eliminarTarjetas() {
        ...
        if (indices.length == 0 && tarjetaSeleccionada == null) { // Si no ha seleccionado ninguna tarjeta
            ...
            // Si selecciono la tarjeta, pero ha perdido el foco del JTable lo añadimos manualmente
            ...
            if (option == JOptionPane.OK_OPTION) { // Si realiza la confirmación
                if (indices.length == 1) {
                    tarjetaSeleccionada = listaTarjetas.get(indices[0]);
                    try {
                        // Borramos la Tarjeta de la BBDD, la lista y del JTable de Tarjetas
                        conexionTarjetaCreditoDAO.destroy(tarjetaSeleccionada.getNumeroTarjeta());
                        listaTarjetas.remove(tarjetaSeleccionada);
                        this.modeloTarjetasCredito.eliminarTarjeta(tarjetaSeleccionada);
                    } catch (NonexistentEntityException ex) {
                        JOptionPane.showMessageDialog(this, "No se ha podido borrar la tarjeta en la base de datos", "Error", JOptionPane.WARNING_MESSAGE);
                    }
                } else {
                    ArrayList<TarjetaCredito> tarjetasAEliminar = new ArrayList<>(); // Array con las tarjetas a eliminar

                    for (int index : indices) { // Obtenemos todas las canciones a eliminar
                        tarjetasAEliminar.add(listaTarjetas.get(index));
                    }
                    // Eliminamos las canciones de la lista y del Modelo

                    for (TarjetaCredito tarjeta : tarjetasAEliminar) {
                        try {
                            this.conexionTarjetaCreditoDAO.destroy(tarjeta.getNumeroTarjeta());
                        } catch (NonexistentEntityException ex) {
                            JOptionPane.showMessageDialog(this, "No se ha podido borrar la tarjeta en la base de datos", "Error", JOptionPane.WARNING_MESSAGE);
                        }
                    }
    ...
    /**
     * Método privado el cual modifica el PIN de una tarjeta.
     * Si no ha seleccionado ninguna tarjeta o ha seleccionado más de una
     * se le informa al usuario. Si tiene seleccionada una le pedimos el 
     * PIN de la tarjeta seleccionada para posteriormente actualizar el PIN
     * por el nuevo introducido.
     * El PIN debe de tener 4 dígitos.
     * Si introduce mal el PIN se le informa y si esta más formato también.
     * Si cancela la operación de modificación se le informa.
     */
    private void modificarPIN() {
        ...
        if (validarSiExisteTarjetaSeleccionada() && solicitarAntiguoPIN(tarjetaSeleccionada)) { // Si ha seleccionado una tarjeta y escribe su pin
            ...
            if (newPin != null) { // Si el PIN es válido
                ...
                try {
                    conexionTarjetaCreditoDAO.edit(tarjetaSeleccionada);
                     JOptionPane.showMessageDialog(rootPane, "Se ha actualizado el PIN correctamente.");

                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(this, "No se ha podido editar la tarjeta en la base de datos", "Error", JOptionPane.WARNING_MESSAGE);
                }
            } else {
    ...
    /**
     * Método privado para realizar un Pago a una Tarjeta.
     * Primero se valida que exista una tarjeta seleccionada 
     * si no ha seleccionado ninguna o ha seleccionado mas de una
     * se le informa al usuario para que seleccione una.
     * Se le pide el PIN de la tarjeta por seguridad una vez introducido
     * Se muestra un JOptionPane para que inserte el Concepto y la Cantidad
     * El concepto no debe de poseer mas de 50 caracteres,
     * Y la cantidad debe de ser inferior o igual a la cantidad disponible de la tarjeta.
     * Si todo es correcto se realiza el pago y se añade el movimiento a la BBDD
     */
    private void realizarPago() {
        if (validarSiExisteTarjetaSeleccionada() && solicitarAntiguoPIN(tarjetaSeleccionada)) {
            ...
            if (pagoValido) { // Si el pago es Válido (Cantidad y Concepto)
                ...
                tarjetaSeleccionada.pagar(Double.parseDouble(cantidad), concepto);
                try {
                    conexionTarjetaCreditoDAO.edit(tarjetaSeleccionada); // Añadimos el pago a la BBDD
                    /* 
                        Obtenemos la Tarjeta Actualizada de la BBDD para evitar duplicidad de movimiento,
                        Ya que al seguir añadiendo movimientos sin cargarla de la BBDD se añaden todos los nuevos 
                        movimientos. EJ: En la BBDD existe 1 movimiento ahora añades 1 movimientos y se añade a la BBDD, pero si
                        vuelves a añadir unis movimientos sin cargarla de nuevo desde la BBDD JPA da a entender que existen 2
                        movimientos nuevos el anterior y el que acabas de insertar en la BBDD se registrará 3 movimientos 
                        y así sucesivamente, por eso se vuelve a cargar el objeto desde la BBDD.
                    */
                    this.listaTarjetas.set(this.listaTarjetas.indexOf(tarjetaSeleccionada), conexionTarjetaCreditoDAO.findTarjetaCreditoJPA(tarjetaSeleccionada.getNumeroTarjeta()));
                    tarjetaSeleccionada = this.listaTarjetas.get(this.listaTarjetas.indexOf(tarjetaSeleccionada));
    ...
    /**
     * Método para actualizar los gastos totales de la tarjeta pasada por parámetro,
     * actualiza el JLabel del JTabbedPane
    ...
    /**
     * Método para actualizar el Modelo del JTable de Tarjetas
     * Limpia la tabla e inserta todas las tarjetas de la Lista de Tarjetas
     */
     ...
     */
    private boolean solicitarAntiguoPIN(TarjetaCreditoJPA tarjeta) {
        boolean validPIN = false;
        boolean exit = false;
        String userPIN;
        do {
    ...
    private String validarPago(String concepto, String cantidad, double saldoDisponible) {
        ...
        if (validPago && Double.parseDouble(cantidad) <= 0) { // Si ha insertado la cantidad negativa le informamos
            errorMessage = "La cantidad no puede ser negativo o 0";
            validPago = false;
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(JFInterfazJPA.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new JFInterfazJPA().setVisible(true);
            }
        });
    }
    ...
    private ArrayList<TarjetaCreditoJPA> listaTarjetas;
    private TarjetaCreditoJPA tarjetaSeleccionada;
